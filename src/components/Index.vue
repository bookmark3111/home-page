<template>
  <div class="content">
    ...
    <div class="contact-section contact-section-hover" v-motion-pop> <span
        style="line-height: 1.75rem;"><b>联系方式（点图标/点头像）！！！</b></span></div>
    <div class="contact-section" v-motion-pop>
      <template v-for="contact in contacts" :key="contact.type">
        <a v-if="contact.url" :href="contact.url" target="_blank" class="contact-item"
          :style="{ '--hover-color': contact.hoverColor }">
          <span v-if="contact.cus != 1"><i :class="contact.icon"></i></span>
          <span v-else>抖音</span>
          <span class="tooltip">{{ contact.type }}</span>
        </a>
        <span v-else @click="toggleQRCode(contact.qrCode)" class="contact-item"
          :style="{ '--hover-color': contact.hoverColor }">
          <span v-if="contact.cus != 1"><i :class="contact.icon"></i></span>
          <span v-else><img src="http://bookmarksoft.cn:53785/oss/douyinx.png" style="max-width: 24px;"></span>
          <span class="tooltip">{{ contact.type }}</span>
        </span>
      </template>
